...
import {
  ...
  structById, shipById, planetById, PIRATE_TEAM,
} from './core';
import { SHIP_CLASSES, DOCK_RANGE, MINES, GADGET_ORDER, MODES } from './data';
...
import { initAudio, playFx, sfx } from './sfx';
...
function handleInput(dt: number) {
  ...
  if (input.pressed('Escape')) {
    ...
    else if (hud.buildOpen || hud.shopOpen) hud.closePanels();
  ...
  // ---------- Tir ----------
  if (!tactical && ship) {
    if (input.down('Space') || (input.leftDown && !hud.ctxOpen && !buildMode)) {
      fireShipWeapon(gs, ship, 0, aim);
    }
    if (input.down('KeyQ')) fireShipWeapon(gs, ship, 1, aim);  // A en AZERTY
    if (input.down('KeyE')) fireShipWeapon(gs, ship, 2, aim);
    if (input.pressed('KeyF') === false && input.down('KeyF')) { /* géré plus bas en continu */ }
  }
  ...
  // ---------- Mines larguées ----------
  for (const _ of input.middleClicks) {
    if (ship && dropMine(gs, ship)) sfx.mine();
    else sfx.error();
  }
  ...
  for (const c of input.clicks) {
    ...
    // clic dans le vide en vue tactique : désélectionne
    if (tactical && !c.ctrl) gs.selection = [];
  }
...
function activateGadgetSmart(gid: GadgetId, aim: V2) {
  ...
  if (gid === 'frappe') {
    // cible : l'ennemi le plus proche du curseur
    let bd = 120;
    for (const s of gs.ships) {
      if (!s.alive || s.team === gs.playerTeam) continue;
    ...
    for (const st of gs.structures) {
      if (!st.alive || st.team === gs.playerTeam) continue;
